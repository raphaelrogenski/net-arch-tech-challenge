...
{
    public class ContactCommandsService
    {
        private readonly IMessageService _messageBus;
        private readonly IContactUniquenessValidator _uniquenessValidator;

        public ContactCommandsService(IMessageService messageBus, IContactUniquenessValidator uniquenessValidator)
        {
        ...
        private void EnsureValidation(ContactDto dto)
        {
            ...
            var nameAlreadyInUse = !nameIsEmpty && _uniquenessValidator.ContactNameAlreadyInUse(dto.Name, dto.Id).Result;
            if (nameAlreadyInUse)
                errorList.Add("Name already in use!");

            var phoneAlreadyInUse = !phoneDDDIsInvalid && !phoneNumberIsInvalid && _uniquenessValidator.ContactPhoneAlreadyInUse(dto.PhoneDDD, dto.PhoneNumber, dto.Id).Result;
            if (phoneAlreadyInUse)
                errorList.Add("Phone already in use!");

            var emailAlreadyInUse = !emailIsInvalid && _uniquenessValidator.ContactEmailAlreadyInUse(dto.EmailAddress, dto.Id).Result;
